...
import { MidgardUrlLD, PoolShare, PoolShareLD, PoolSharesLD } from '../midgardTypes'
          ...
          A.filterMap<MemberPool, PoolShare>(({ pool, liquidityUnits, runeAddress, assetAddress, assetAdded }) =>
            ...
            FP.pipe(
              ...
              O.map((asset) => ({
                ...
                // BaseAmount of added asset - Note: Mayachain treats all assets as 1e8 decimal based
                assetAddedAmount: baseAmount(bnOrZero(assetAdded), CACAO_DECIMAL)
              }))
            )
          )
        )
