...
import { Client$, Client as BaseClient } from '../evm/types'
...
export const createTransactionService = (client$: Client$, network$: Network$): TransactionService => {
  ...
  const runSendPoolTx$ = (client: BaseClient, { ...params }: SendPoolTxParams): TxHashLD => {
    // helper for failures
  ...
  const approveERC20Token$ = (params: ApproveParams): TxHashLD => {
    ...
    if (network === Network.Mainnet && !addressInBaseWhitelist(contractAddress))
      return Rx.of(
        RD.failure({
