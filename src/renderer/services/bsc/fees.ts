...
export const createFeesService = (client$: Client$): FeesService => {
  ...
  async function estimateAndCalculateFees(client: Client, params: TxParams) {
    ...
    const fees: Fees = {
      ...
      average: getFee({ gasPrice: averageGP, gasLimit, decimals: BSC_GAS_ASSET_DECIMAL }),
      fast: getFee({ gasPrice: fastGP, gasLimit, decimals: BSC_GAS_ASSET_DECIMAL }),
      fastest: getFee({ gasPrice: fastestGP, gasLimit, decimals: BSC_GAS_ASSET_DECIMAL })
    }
    // Calculate fees
    return fees
  }
            ...
            (client) =>
              Rx.combineLatest([
                ...
                RxOp.map(([gasLimit, gasPrices]) =>
                  getFee({ gasPrice: gasPrices.fast, gasLimit, decimals: BSC_GAS_ASSET_DECIMAL })
                ),
                RxOp.map(RD.success),
                RxOp.catchError((error) => Rx.of(RD.failure(error))),
                RxOp.startWith(RD.pending)
